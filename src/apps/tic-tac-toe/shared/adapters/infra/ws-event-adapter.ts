...
import type { IEventAdapter, IEventType } from "@/apps/tic-tac-toe/shared/adapters/domain";
...
export class WsEventAdapter implements IEventAdapter {
    ...
    async publish(id: string, event: IEventType, message?: string): Promise<void> {
    ...
    async _send(conn: WebSocket, event: IEventType, message?: string): Promise<void> {
        const payload = {
            event,
            message: message,
        };
